...
function checkForRedirects() {
	$checkRedirectsTable = mysql_query("");
...
function titleExists($product_selector) {
	...
	$checkPage = mysql_query("SELECT * FROM xxxxx WHERE selector = '".$product_selector."' and meta_title is not null"); 	
	$checkSub = mysql_query("SELECT * FROM xxxx WHERE url = '".$product_selector."' and meta_title is not null"); 
	$checkCat = mysql_query("SELECT * FROM xxxxx WHERE url = '".$product_selector."' and meta_title is not null");
	$checkForDirectory = mysql_query("SELECT * FROM xxxxx WHERE request_uri = '".$_SERVER['REQUEST_URI']."'");
...
function descExists($product_selector) {
	...
	$checkPage = mysql_query("SELECT * FROM xxxxxx WHERE selector = '".$product_selector."' and meta_title is not null"); 	
	$checkSub = mysql_query("SELECT * FROM xxxxxx WHERE url = '".$product_selector."' and meta_title is not null"); 
	$checkCat = mysql_query("SELECT * FROM xxxxxx WHERE url = '".$product_selector."' and meta_title is not null"); 
	$checkForDirectory = mysql_query("SELECT * FROM xxxxxx WHERE request_uri = '".$_SERVER['REQUEST_URI']."'");
  $rCheckForDirectory = mysql_fetch_assoc($checkForDirectory);
	if(mysql_num_rows($checkPage) > 0){
		if(strpos($host,'gallery') !== false) {
 			$rCheckPage = mysql_fetch_assoc( $checkPage );
...
function checkRedirect( $base, $dir, $page ) {
	...
	$getPaths = mysql_query( "SELECT * FROM xxxxxx WHERE url_old = '" . $path . "'" );
...
function categories( $cats ) {
	...
	foreach( $categories as $category ) {
		...
		echo "<a class='animate' itemprop='category' content='" . $category . "' >" . $category . "</a>";
	}
}
...
function allProducts() {
	...
	while( $main = mysql_fetch_assoc( $getMain ) ) {
		...
		if( $main['is_visible'] == 1 ) {
			...
			$getCat = mysql_query( 'SELECT * FROM xxxxxx WHERE parent = "' . $main['selector'] . '" ORDER BY oid' );
			while( $cat = mysql_fetch_assoc( $getCat ) ) {

				echo '<ul class="products-main-sub">';
				echo '<h2>' . $cat['name'] . '</h2>';

				$getItem = mysql_query( 'SELECT * FROM xxxxxx WHERE parent = "' . $cat['selector'] . '"' );
...
function featuredProducts( $l ) {
	...
	$a = mysql_query( 'SELECT * FROM xxxxxxxxxxxx WHERE is_featured = 1 ORDER BY rand()' );
	...
	while( $ab = mysql_fetch_assoc( $a ) ) {
		...
		if( $i > $l ) {
			...
			$b = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $ab['parent'] . '"' );
...
function relatedProducts( $selector, $limit = 9 ) {
	...
	$getViewedItem = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $selector . '"' );
	$rGetViewedItem = mysql_fetch_assoc( $getViewedItem );
	// explode related products into an array
	$explodedRelatedProducts = explode( ',', $rGetViewedItem['related'] );
	// randomize the array to mix things up a bit
	$randomExplodedRelated = shuffle( $explodedRelatedProducts);
	// set counter to -1
	$i = -1;
	// make sure there are related products to show
	if( count( $explodedRelatedProducts ) >= 1 ) {
		echo "<h1>Customers Also Bought ";
		//if only one related product, do not display refresh button
		if( count( $explodedRelatedProducts ) == 1 ) { } else {
			echo "<i class='fa fa-lg fa-refresh refresh-list animate'></i>";
		}
		echo "</h1>";
		echo "<ul class='refresh'>";
		// return each related product
		foreach( $explodedRelatedProducts as $d ){
			// initialize counter
			$i++;
			...
			if( $i <= $limit || $limit == 0 ) {
				...
				$getRelated = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $selectorTrimmed . '"' );
				$related = mysql_fetch_assoc($getRelated);
				// select the sub category and main category of the current related product
				$getCategory = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $related['parent'] . '"' );
				$category = mysql_fetch_assoc($getCategory);
				if($category === false) {
					$getMainRelated = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $related['parent'] . '"' );
				...
				if($related['is_visible'] == 1){
					echo "<li class='related-product'>
			                ...
			                <img alt='".$rGetViewedItem['name']." related product: ".$related['name']."' src='".$path."'/>
...
function slideText( $source = 'product', $selector, $slide, $financed ) {
	...
	$getSlide = mysql_query( "SELECT * FROM xxxxxx WHERE is_visible = 1 AND selector = '" . $selector . "' AND slide = '" . $slide . "'" );
...
function slides( $source = 'product', $selector, $limit ) {
	...
	if( $source == 'product' ) {
		...
		// select the provided product from the database
		$getProduct = mysql_query( 'SELECT * FROM xxxxxx WHERE url = "' . $selector . '"' );
		$product = mysql_fetch_assoc($getProduct);
		// get the directory for the products slide images
	...
	if( is_dir( $path ) ) {
		...
		while( $slide = readdir( $handle ) ) {
			...
			if( $slide != "." && $slide != ".." && $slide != "Thumbs.db" ) {
				...
				$getSlideTitle = mysql_query("SELECT * FROM xxxxxx WHERE is_visible = 1 AND selector = '".$selector."' AND slide = '".$i."'");
...
function rating( $source = 'product', $selector ) {
	...
	if( $source == 'product' ) {
		$getRatings = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $selector . '"' );
	} else {
		$getRatings = mysql_query( 'SELECT * FROM xxxxxx WHERE selector = "' . $selector . '"' );
	}
...
function timeline() {
	...
	$getEvents = mysql_query( 'SELECT * FROM xxxxxx ORDER BY date DESC' );
...
function testimonials( $source = 'product', $selector = null, $limit ) {
	...
	if( $source == 'all' || $source == 'product' ) {
		if( $selector == null ) {
			...
			$getTestimonials = mysql_query( 'SELECT * FROM xxxxxx WHERE is_approved = 1 AND is_visible = 1 ORDER BY rand()' );
		} else {
			// limit the selection to the selected product
			$getTestimonials = mysql_query( 'SELECT * FROM xxxxxx WHERE is_approved = 1 AND is_visible = 1 AND selector = "' . $selector . '" ORDER BY rand()' );
		}
			...
			if( $i > $limit ) {
				...
				echo "<div class='testimonial-content'>";
					...
					if( $testimonial['selector'] != 'null' ) {
						$getProduct = mysql_query( "SELECT * FROM xxxxxx WHERE selector = '" . $testimonial['selector'] . "'" );
						$product = mysql_fetch_assoc( $getProduct );
...
function features( $source = 'product', $selector, $limit = 0 ) {
	...
	$getFeatures = mysql_query( "SELECT * FROM xxxxxx WHERE is_visible = 1 ORDER BY oid ASC" );
...
function accessories( $accessory_selector ) {
	...
	$getAccessories = mysql_query( "SELECT * FROM xxxxxx WHERE selector = '" . $accessory_selector . "'" );
...
function clearance() {
	$getClearance = mysql_query('SELECT * FROM xxxxxx');
	...
	while( $rClearance = mysql_fetch_assoc( $getClearance ) ) {
		...
		if($rClearance['combo'] == 1) {
			...
			if($rClearance['no_pdf'] == 1) { } else {
				echo "<a class='pdf-link' target='_blank' href=". DIR_PDFS . 'clearance' . '/' . $comboName . '/' . $comboName .'.pdf' . ">View PDF</a></p>" . '<br />';
			}
		} else {
...
function galleryThumbs( $source = 'product', $selector, $limit = 15 ) {
	if( $source == 'product' ) {
		...
		$hasGalVideo = mysql_query( "SELECT * FROM xxxxxx WHERE selector = '" . $selector . "'" );
		...
		$getPackage = mysql_query( "SELECT * FROM xxxxxx WHERE selector = '" . $selector . "'" );
		...
		$getCustom = mysql_query( "SELECT * FROM xxxxxx WHERE selector = '" . $selector . "'" );
	...
	foreach($list as $image){
		...
		$checkHash = mysql_query( "SELECT * FROM xxxxxx WHERE thumbhash = '" . $currentHash . "' and imgname = '" . $matchedImageName . "' and selector = '" . $selector . "'" );
...
function clearanceThumbs($id,$comboName) {
	// select the product from the database
	$getClearance = mysql_query( "SELECT * FROM xxxxxx where id = '" . $id . "'" );
	$rClearance = mysql_fetch_assoc($getClearance);
	// set the path for the products gallery
	// check if the path exists
	if($rClearance['combo'] == 1) {
...
function makeFeatureThumbs( $selector) {
	...
	$getFeatureInfo = mysql_query( "SELECT selector FROM xxxxxx where parent like '%" . $selector . "%'" );
	...
	while( $feature = mysql_fetch_assoc( $getFeatureInfo ) ) {
		...
		foreach($featList as $featImage){
			...
			$featThumb = '/usr/bin/convert' . ' ' . BASE_THUMBNAIL . $featPath . $feat_path_parts['filename'] . '.' . $feat_path_parts['extension'] . ' -resize 210x ' . BASE_THUMBNAIL . $featPath . '_thumbnails/' . $feat_path_parts['filename'] . '_thumb' . '.' . $feat_path_parts['extension'];
				exec($featThumb);
				$addHash = mysql_query("INSERT INTO xxxxxx (thumbhash, selector, imgname) VALUES ('$featCurrentHash','$selector','$currentFile')");
...
function showVideo( $source = 'product', $selector ) {
	...
	if( $source == 'product' ) {
		$getVideos = mysql_query( "SELECT * FROM xxxxxx WHERE is_visible = 1 AND selector = '" . $selector . "' LIMIT 1" );
	} elseif( $source == 'package' ) {
		$getVideos = mysql_query( "SELECT * FROM xxxxxx WHERE is_visible = 1 AND selector = '" . $selector . "' LIMIT 1" );
	} elseif( $source == 'sub' ) {
		$getVideos = mysql_query( "SELECT * FROM xxxxxx WHERE url = '" . $selector . "' LIMIT 1" );
	}
