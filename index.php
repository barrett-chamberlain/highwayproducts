...
<div class='container sb'>
    ...
    $getMainCat = mysql_query('SELECT * FROM xxxxxx WHERE url = "'.$product_selector.'"');
    ...
    <?php }else{
    $getSub = mysql_query('SELECT * FROM xxxxxx WHERE url = "'.$product_selector.'"');
    if(mysql_num_rows($getSub) > 0){
        $sub = mysql_fetch_assoc($getSub);
        $getSubCat = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$sub['parent'].'"');
    ...
    $getProduct = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$product_selector.'" LIMIT 1');
    ...
    $getVideo = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$product_selector.'" LIMIT 1');
    $video = mysql_fetch_assoc($getVideo);
    $getCategory = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$product['parent'].'"');
    ...
    echo '<script type="application/ld+json">
{
  ...
  "brand": {
    ...
    if($product['is_base'] == 1) { 
        $getMainCategory = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$product['parent'].'"');
    } else {
        $getMainCategory = mysql_query('SELECT * FROM xxxxxx WHERE selector = "'.$category['parent'].'"');
    }
...
<gcse:searchbox-only></gcse:searchbox-only>
            ...
            <div class='order-direct order-mobile'>
                ...
                <p class='order-text'>
                    ...
                    <span class='call-us-number'><a onclick="ga('send', 'event', 'Link', 'Call Us Now', 'Action Value');" href='<?php echo PHONE_NUMBER_LINK ?>'>1-877-690-4679</a></span>
                </p>
            </div>
    ...
    <div class='right-content'>
        ...
        <div class='sidebar-signup'>
            ...
            <script type="text/javascript">(function(d, s, id) {
                ...
                if (d.getElementById(id)) return;
                js = d.createElement(s); js.id = id;
                js.src = "//forms.aweber.com/form/99/<?php echo $; ?>.js";
                fjs.parentNode.insertBefore(js, fjs);
                }(document, "script", "aweber-wjs-"));
            </script>
...
<?php // include addthis api if sharing is allowed
if( SET_SHARING == 'true' ) { ?>
    <script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pub"></script>
<?php } ?>
...
<gcse:searchbox-only></gcse:searchbox-only>
            ...
            <iframe class='intro-video player youtube' height='0' src='https://www.youtube.com/embed/' allowfullscreen></iframe>
    ...
    fjs.parentNode.insertBefore(js, fjs);
        ...
        <div class='right-content'>
            ...
            if( SET_TWITTER_FEED == 'true' ) { ?>
                <a class='twitter-timeline hide eight_hundred' style="height: 400px;" href='https://twitter.com/HighwayProducts' data-widget-id="">Tweets by @HighwayProducts</a>
